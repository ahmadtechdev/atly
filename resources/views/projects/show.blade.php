...
@php
    $viewer = auth()->user();
    $canManage = $viewer && $project->canManage($viewer);
    $isOwner = $viewer && $viewer->id === $project->user_id;
@endphp
                ...
                <div class="flex flex-wrap items-center gap-2">
                    @if ($canManage)
                        ...
                        <button
                            ...
                            data-invitable-type="project"
                            data-invitable-id="{{ $project->id }}"
                            data-invitable-label="{{ $project->name }}"
        ...
        <section class="rounded-atly-lg border border-atly-border bg-atly-card shadow-atly">
            <header class="flex flex-wrap items-center justify-between gap-2 border-b border-atly-border px-5 py-4">
                ...
                <div class="flex items-center gap-2">
                    @php
                        $canAddTasks = $viewer && ($canManage || $project->roleFor($viewer)?->canComplete()) && ! $project->isCompleted();
                    @endphp
                    @if ($canAddTasks)
                        <button
                            type="button"
